...
test('Clicks complete goal', async () => {
  ...
  await page.waitForSelector('aria/Goal not completed');
...
test('Deletes goal cleanup', async () => {
  await logIn(page, 'e2etester@gmail.com', 'password');
  await page.hover('aria/Goals dropdown');
  await page.waitForSelector('aria/View Goal Button');
  ...
  await searchInput.type('Learn React');
  ...
  // Re-select the 'Delete Goal' button before clicking it again
  const deleteGoalButton = await page.$('aria/Delete Goal');
